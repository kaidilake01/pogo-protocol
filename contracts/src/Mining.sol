...
interface IFixedAllocationFunding {
...
interface IFixedAllocationDividends { function syncStake(address account) external; }
interface IFixedAllocationPair is IERC20 {
...
contract FixedAllocationMining is ReentrancyGuard {
    ...
    function _stake(uint8 id,uint256 amount) private {
        ...
        if(id<2)IFixedAllocationDividends(vault).syncStake(msg.sender);
    ...
    function assetPriceUSD(uint8 id) external view returns(uint256) {
        _check(id);uint256 price=IFixedAllocationFunding(fundingPool).stakingPriceUSD();
        if(id<2)return price;
        (uint112 r0,uint112 r1,)=IFixedAllocationPair(lpToken).getReserves();
        uint256 reserve=IFixedAllocationPair(lpToken).token0()==address(token)?r0:r1;
        return Math.mulDiv(reserve*2,price,IFixedAllocationPair(lpToken).totalSupply());
    }
    ...
    function _payAll(address account) private {
        for(uint8 i;i<3;i++){
            ...
            IFixedAllocationFunding(fundingPool).releaseMiningReward(account,amount);emit RewardPaid(account,i,amount);
        }
    }
}
contract FixedAllocationMiningDeployer {
    function deploy(address token,address vault) external returns(address) { return address(new FixedAllocationMining(token,vault,msg.sender)); }
}
