...
import {LaunchTypes, ILaunchTokenV3} from "../internal/LaunchTypes.sol";
import {MultiAssetCurve} from "../internal/MultiAssetCurve.sol";
import {IV2Router, IV2Pair, IWBNB} from "../internal/Interfaces.sol";
...
contract DeferredMiningRevenueVault is ReentrancyGuard {
    ...
    function setStakingPool(address staking_) external {
        ...
        // A counterfactual address may hold tokens before its mining contract is deployed.
        // Settle its past credit and remove its holder weight before excluding it.
        _sync(staking_,0);
...
contract DeferredMiningVaultDeployer {
    ...
    function deploy() external returns(address){if(msg.sender!=factory)revert Unauthorized();return address(new DeferredMiningRevenueVault(factory));}
}
