...
At `GraduationV3`, track the emitted pair and index its DEX swap events. Version-15 DeFi curves also deploy their mining contract in that transaction and emit `MiningFunded(stakingPool, rewards)`. Index that event or re-read the curve’s `stakingPool()` after graduation. A zero address recorded at creation is expected and must not become a permanent cache result. Source-verification workers should retain a bounded retry queue for deferred pools after advancing the token-creation cursor. Internal history is not automatically available to third-party terminals: they must integrate these events or an indexer. Merely verifying a token on BscScan does not integrate its internal chart with every trading terminal.
