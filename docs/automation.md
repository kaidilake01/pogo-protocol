...
New creator revenue and holder dividends are denominated only in the project's paired asset. For BNB pairs, recipients receive native BNB; for supported non-BNB pairs, including PEPE, they receive that paired token. Mining emissions remain project tokens.
...
See the current [revenue vault implementation](../contracts/src/RevenueVault.sol) for settlement checks and payout behavior.
