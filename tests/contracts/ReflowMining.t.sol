...
import {ReflowMining} from "../../contracts/src/v11/ReflowMining.sol";

contract ReflowPairMock is MockAsset {
...
contract ReflowFundingMock {
    MockAsset public token;ReflowMining public mining;ReflowPairMock public pair;
    address public treasury=address(0xfee);
    bool public failing;bool public missingPrice;
    constructor(){token=new MockAsset();pair=new ReflowPairMock(address(token));}
    function initialize() external {require(address(mining)==address(0));mining=new ReflowMining(address(token),address(this),address(this));}
...
interface VmReflowTime { function getBlockTimestamp() external view returns(uint256); }
contract ReflowMiningTest is TestBase {
    function advance(uint256 seconds_) private {vm.warp(VmReflowTime(address(vm)).getBlockTimestamp()+seconds_);}
    ReflowFundingMock f;ReflowMining m;MockAsset t;ReflowPairMock lp;
    address constant A=address(0xa11ce);address constant B=address(0xb0b);
    uint256 constant BUDGET=180_000_000 ether;
    function setUp() public {
        vm.warp(1_000_000);f=new ReflowFundingMock();f.initialize();f.start(BUDGET);m=f.mining();t=f.token();lp=f.pair();
    ...
    function testSplitAndDayEmissionsNoCrossPoolWeights() public {
        assertEq(m.poolState(0).budget,1_800_000 ether);assertEq(m.poolState(1).budget,16_200_000 ether);assertEq(m.poolState(2).budget,162_000_000 ether);
        allStake(A,100 ether);advance(1 days);
        assertEq(m.earnedIn(0,A),20_000 ether);assertEq(m.earnedIn(1,A),180_000 ether);assertEq(m.earnedIn(2,A),1_800_000 ether);
    ...
    function testIndependentEmptyClocksAndNoRetroactiveJackpot() public {
        ...
        assertEq(m.poolState(0).claimed,1_800_000 ether);assertEq(m.poolState(2).activeSeconds,80 days);
        assertEq(m.poolState(1).scheduled,0);assertEq(m.poolEndsAt(1),0);
        advance(10 days);m.claimFor(B);assertEq(m.poolState(2).claimed,162_000_000 ether);
        vm.expectRevert(ReflowMining.Inactive.selector);vm.prank(B);m.stakeLP(1);
    ...
    function testDilutionOnlyAffectsFutureEarnings() public {
        ...
        vm.prank(B);m.stakeLP(300 ether);assertEq(m.earnedIn(2,A),1_800_000 ether);assertEq(m.earnedIn(2,B),0);
        advance(1 days);assertEq(m.earnedIn(2,A),2_250_000 ether);assertEq(m.earnedIn(2,B),1_350_000 ether);
    }
    function testTwentyFourHourIndependentLocksAndPartialExits() public {
        ...
        vm.warp(start+24 hours-1);vm.expectRevert(ReflowMining.Locked.selector);vm.prank(A);m.withdrawLocked(0,1);
        ...
        vm.expectRevert(ReflowMining.Locked.selector);vm.prank(A);m.withdrawLocked(1,1);
    ...
    function testOnlyCanonicalLPAndNoUnapprovedWithdrawOrActivation() public {
        ...
        vm.expectRevert(ReflowMining.Unauthorized.selector);m.activate(100,address(fake));
        vm.expectRevert(ReflowMining.Unauthorized.selector);m.payReward(A);
        vm.expectRevert(ReflowMining.InvalidConfig.selector);f.start(100);
        vm.expectRevert(ReflowMining.InvalidAmount.selector);vm.prank(B);m.withdrawLP(0,1);
        vm.expectRevert(ReflowMining.InvalidConfig.selector);m.poolState(3);
    }
    function testEarlyExitPenaltyForSingleAndLPIsPermanentlyLocked() public {
        allStake(A,100 ether);advance(12 hours);
        vm.expectRevert(ReflowMining.Locked.selector);vm.prank(A);m.withdrawLP(0,10 ether);
    ...
    function testLPAdditionalDepositDoesNotExtendOldLock() public {
        ...
        vm.expectRevert(ReflowMining.Locked.selector);vm.prank(A);m.withdrawLP(1,100 ether);
