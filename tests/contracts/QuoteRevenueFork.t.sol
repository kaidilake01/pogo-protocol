...
import {ReflowMining,ReflowMiningDeployer} from "../../contracts/src/v11/ReflowMining.sol";
...
contract QuoteRevenueForkTest is TestBase {
 ...
 function testQuoteRevenueBscV2AllSupportedAssetsAndThreePools() public {
  ...
  ReflowMiningDeployer md=new ReflowMiningDeployer();ReflowCurveDeployer cd=new ReflowCurveDeployer(address(f),address(md));
  address vaultDeployer=address(new QuoteVaultDeployer(address(f)));
  ...
  for(uint256 j;j<4;j++)for(uint256 taxed;taxed<2;taxed++){
   ...
   (,address pool,address vault,)=f.projects(token);ReflowCurve c=ReflowCurve(pool);ReflowMining m=ReflowMining(c.stakingPool());
   assertEq(c.VERSION(),11);assertEq(m.VERSION(),11);assertEq(m.penaltyRecipient(),address(0xdead));
...
assertEq(m.lpToken(),pair);assertTrue(IERC20(pair).balanceOf(address(0xdead))>0);
